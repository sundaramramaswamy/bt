...
- **Header probe** (`HeaderProbe`): `BuildCommand`/`DirtyCommand` (and therefore `WatchCommand` via `RunBuild`) call `HeaderProbe.Enrich(graph)` before any dirty check.  Two classes of sources are probed:
  - *Inferred sources*: inference synthesises a CL command for each new `.cpp` but has no tlog data, so headers are unwired by default.
  - *Stale-tlog sources*: existing CL commands whose `.cpp` or any tlog-known header has mtime > binlog timestamp.  bt never writes tlogs, so any `#include` added during a prior bt session is invisible to `FileToConsumers` until reprobed.  Without this, a later layout change to the unseen header silently leaves the `.obj` stale → ODR heap corruption.
  For each probed source: (1) copy `#include` edges from the project's `/Yc pch.cpp` invocation (already wired by `WireTlogHeaders`) — this is the only source of PCH-internal headers, since `/Yu` peers' tlogs lack them too (cl reads the `.pch` blob without re-parsing); (2) strip `/Yu`/`/Fp` flags and spawn `cl /Y- /showIncludes /Zs` with GlobalEnv+ProjectEnv layered — disabling PCH so the probe doesn't depend on a `.pch` file; (3) parse `Note: including file:` lines, dedupe vs already-wired headers, wire as synthetic `#include` cmds (id prefix `#include#probe#`).  No conservative fallback: `/showIncludes /Zs` failures are a strict subset of real-build failures (it only does preprocess + parse, no codegen), so if it errors out the next `bt build` would fail too — print cl's stderr and throw.  Query commands (`bt srcs`, `bt bins`, `bt graph`) deliberately skip enrichment — they don't need header edges.
