...
- **Inferred-source header probe** (`InferredHeaderProbe`): inference synthesises a CL command for each new `.cpp` but has no tlog data, so headers are unwired by default.  `BuildCommand`/`DirtyCommand` (and therefore `WatchCommand` via `RunBuild`) call `InferredHeaderProbe.Enrich(graph)` before any dirty check.  For each inferred CL: (1) copy `#include` edges from the project's `/Yc pch.cpp` invocation (already wired by `WireTlogHeaders`) — this is the only source of PCH-internal headers, since `/Yu` peers' tlogs lack them too (cl reads the `.pch` blob without re-parsing); (2) spawn `cl /showIncludes /Zs <newSrc>` with the inferred command line + GlobalEnv+ProjectEnv layered (mirrors `BuildCommand.ExecuteCommand`); (3) parse `Note: including file:` lines from stdout, dedupe vs PCH copies, wire as synthetic `#include` cmds (id prefix `#include#probe#`).  No conservative fallback: `/showIncludes /Zs` failures are a strict subset of real-build failures (it only does preprocess + parse, no codegen), so if it errors out the next `bt build` would fail too — print cl's stderr and throw.  Query commands (`bt srcs`, `bt bins`, `bt graph`, `bt cmds`) deliberately skip enrichment — they don't need header edges.
