...
- **LINK side-output `.pdb`**: LINK declares a single `OutputFile` (the `.dll`/`.exe`) but also produces a `.pdb` sibling (when debug info is enabled) via the `ProgramDatabaseFile` task parameter.  bt registers the PDB as a second output of the same LINK command (shared `cmdId`, appended to `Outputs`) so downstream Copy tasks — notably Binplace copying to `Symbols\Product\` — wire up, and mtime dirty checks propagate.  `.pdb` is classified as a dev-visible `FileKind.Output` so it appears in `graph -f` as a sibling branch to the `.dll`, and `build`/`dirty <pdb>` resolve the backward cone via `GetCommandCone`.
